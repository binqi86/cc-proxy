...
export default function TrayPopup() {
  ...
  useEffect(() => {
    ...
    resizeTimer.current = setTimeout(async () => {
      try {
        ...
        const count = configuredProviders.length;
        // Header: 44px, section title: 22px, each provider: 32px, footer: 40px, padding: 20px
        const contentH = count > 0 ? (22 + count * 32 + 8) : 36;
        const h = Math.min(44 + contentH + 40 + 4, 440);
  ...
  return (
    <div className="flex flex-col h-screen select-none rounded-2xl overflow-hidden" style={{ background: `rgb(var(--card))`, border: `1px solid rgb(var(--border) / 0.6)`, boxShadow: '0 20px 50px rgba(0,0,0,0.3)' }}>
      {/* Header — brand left, service+theme icon buttons right */}
      <div className="flex items-center gap-3 px-4 py-2.5 shrink-0 border-b border-border/30">
      ...
      {/* Footer */}
      <div className="flex items-center gap-2 px-4 py-2.5 shrink-0 border-t border-border/20">
